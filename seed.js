...
async function seed() {
    ...
    for (const pedido of SEED_DATA) {
        ...
        await dbRun(
            ...
            [pedido.pedido_id, pedido.token, pedido.cliente, pedido.cnpj, pedido.revenda]
        );
        ...
        console.log(`  ✅ Pedido ${pedido.pedido_id} inserido (${pedido.licencas.length} licenças)`);
    }

    console.log('\n🎉 Seed concluído! Pedidos de teste disponíveis:');
    console.log('');
    SEED_DATA.forEach(p => {
        console.log(`  📋 ${p.pedido_id} → http://localhost:3000/?pedidoId=${p.pedido_id}&token=${p.token}&revenda=${p.revenda}`);
    });
    console.log('');
